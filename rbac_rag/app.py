...
from .rbac import ensure_widgets, get_allowed_domains, list_role_ids, resolve_selected_role
...
def create_app(
    *,
    ...
    role_ids = list_role_ids(spark)
    ensure_widgets(dbutils, role_ids)

    selected_role = resolve_selected_role(dbutils)
    rbac_enabled = dbutils.widgets.get("rbac_enabled") == "ON"
    post_check_enabled = dbutils.widgets.get("post_check") == "ON"
    allowed_domains = get_allowed_domains(spark, selected_role) if rbac_enabled else []
